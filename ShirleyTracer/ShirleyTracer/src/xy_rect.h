...
public:
	...
	bool hit(const ray& r, float t0, float t1, hit_record& rec) const;
	bool hitP(const ray& r, float t0, float t1, hit_record& rec) const;
...
bool xz_rect::hitP(const ray& r, float t0, float t1, hit_record& rec) const {
	float t = (k - r.origin().y()) / r.direction().y();
	if (t<t0 || t>t1)
		return false;
	float x = r.origin().x() + t * r.direction().x();
